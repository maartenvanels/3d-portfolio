...
The defining features are a three-axle carrier, long single lift cabin, telescoping closed mast, triangular folding jib and carrier-level counterweight. Public dimensions informed the proportions; small details and mechanical connections are simplified.

## Web export

The static structure is merged by material. The hook block and four hoist ropes have separate origins so the rope length follows the hook during opt-in animation. There are no texture maps or remote decoder dependencies. Automated checks enforce a 1.2 MB file budget, fewer than 30,000 triangles and at most 12 mesh draws, and verify the hoist connection at both travel limits.
